...
class RegistrationPageBuyer extends StatefulWidget {
  const RegistrationPageBuyer({Key? key}) : super(key: key);

  @override
  State<RegistrationPageBuyer> createState() => _RegistrationPageBuyerState();
}

class _RegistrationPageBuyerState extends State<RegistrationPageBuyer> {
  ...
  Widget build(BuildContext context) {
    ...
    final qualificationField = TextFormField(
        autofocus: false,
        controller: qualificationEditingController,
        keyboardType: TextInputType.name,
        validator: (value) {
          if (value!.isEmpty) {
        ...
        onSaved: (value) {
          qualificationEditingController.text = value!;
        },
        textInputAction: TextInputAction.next,
        decoration: InputDecoration(
          prefixIcon: const Icon(Icons.account_circle),
          contentPadding: const EdgeInsets.fromLTRB(20, 15, 20, 15),
          hintText: "Enter You Qualification",
          border: OutlineInputBorder(
            borderRadius: BorderRadius.circular(10),
          ),
        ));
...
//Experience
  ...
  postDetailsToFirestore() async {
    ...
    Navigator.pushAndRemoveUntil(
        (context),
        MaterialPageRoute(builder: (context) => DashBoardseller()),
        (route) => false);
  }
}
